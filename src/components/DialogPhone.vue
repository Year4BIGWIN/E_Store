      ...
      <div class="w-2/3">
        <form @submit.prevent="handleSubmit">
          ...
          <div class="w-full grid grid-cols-2 gap-2">
            ...
            <Input label="Operating System :" v-model="formData.operatingSystem" />
          </div>
...
watch(() => props.showDialog, (newVal) => newVal && resetForm());
...
function resetForm() {
  Object.keys(formData).forEach(key => formData[key] = typeof formData[key] === "object" ? {} : "");
  selectedType.value = "";
...
async function handleSubmit() {
  try {
    if (!formData.image_url || !formData.model || !selectedType.value || !selectedBrand.value) {
