...
const fetchSearch = async (keyword) => {
  try {
    const response = await fetch(
      `${apiUrl}/product/search?keyword=${encodeURIComponent(keyword)}`,
      {
        method: "GET",
        headers: {
  ...
  <form @submit="handleSearchSubmit" class="w-full relative">
    ...
    >
      <ul class="py-1">
        ...
        >
          <div
            class="w-8 h-8 sm:w-10 sm:h-10 flex-shrink-0 mr-3 bg-gray-200 rounded-md overflow-hidden"
          >
            <img
              v-if="result.firstImageUrl"
    ...
    <div
      ...
      class="absolute mt-1 w-full bg-white rounded-md shadow-lg z-50 border border-gray-200 p-3"
    >
      <Loader />
